...
function migrateLegacyRoomRegistrationsInFile(dbPath: string): {
  ...
  try {
    ...
    migrationDb.transaction(() => {
      const rows = getPendingLegacyRegisteredGroupJids(migrationDb).map(
        (jid) => ({ jid }),
      );

      for (const row of rows) {
        const plan = buildLegacyRoomMigrationPlan(migrationDb, row.jid);
        if (!plan) continue;
...
describe('room assignment writes', () => {
  ...
  it('does not write legacy registered_groups rows when assigning a canonical room', () => {
  ...
  it('requires explicit migration before init when room_settings conflicts with legacy room-level metadata', () => {
    ...
    expect(getPendingLegacyRegisteredGroupJids(pendingDb)).toEqual([
      'dc:mixed-conflict',
    ]);
    pendingDb.close();
    expect(() => _initTestDatabaseFromFile(dbPath)).toThrow(
  ...
  it('clears stale legacy registered_groups rows after canonical room metadata changes', () => {
