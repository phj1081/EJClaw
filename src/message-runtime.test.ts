...
describe('createMessageRuntime', () => {
  ...
  it('ignores untriggered bot messages in paired rooms', async () => {
  ...
  it('processes triggered bot messages in paired rooms', async () => {
    const chatJid = 'group@test';
    const group = makeGroup('codex');
    const channel = makeChannel(chatJid);
    const saveState = vi.fn();
    ...
    vi.mocked(agentRunner.runAgentProcess).mockImplementation(
      async (_group, _input, _onProcess, onOutput) => {
        await onOutput?.({
          status: 'success',
          result: '확인했습니다.',
          newSessionId: 'session-triggered-bot',
        });
        return {
          status: 'success',
          result: '확인했습니다.',
    ...
    const result = await runtime.processGroupMessages(chatJid, {
      runId: 'run-triggered-bot-message',
      reason: 'messages',
    });

    expect(result).toBe(true);
    expect(agentRunner.runAgentProcess).toHaveBeenCalledTimes(1);
    expect(channel.sendMessage).toHaveBeenCalledWith(chatJid, '확인했습니다.');
