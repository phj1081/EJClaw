...
describe('executeMessageAgentAttemptLifecycle', () => {
  it('keeps the resumed session on the first transient same-account retry', async () => {
    let currentSessionId: string | undefined = 'kept-session';
    ...
    expect(sessionIdsSeenByAttempts).toEqual(['kept-session', 'kept-session']);
    expect(clearStoredSession).not.toHaveBeenCalled();
  });

  it('clears a possibly poisoned resume session only before the final transient retry', async () => {
    ...
    expect(result).toBe('success');
    expect(sessionIdsSeenByAttempts).toEqual([
      'poisoned-session',
      'poisoned-session',
      undefined,
    ]);
