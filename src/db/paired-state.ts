...
interface ClaimPairedTurnReservationArgs {
  chatJid: string;
  taskId: string;
...
function acquirePairedTaskExecutionLease(
  database: Database,
  args: ClaimPairedTurnReservationArgs,
...
function attachAttemptAndCompleteReservation(
  database: Database,
  args: ClaimPairedTurnReservationArgs,
  attempt: { attempt_id: string; attempt_no: number },
  now: string,
): void {
  const turnAttemptNo = attempt.attempt_no;
  const turnAttemptId = attempt.attempt_id;
...
export function claimPairedTurnReservationInDatabase(
  ...
  const tx = database.transaction(() => {
    ...
    acquirePairedTaskExecutionLease(
      database,
      args,
      turnIdentity,
      now,
    ...
    attachAttemptAndCompleteReservation(database, args, currentAttempt, now);
  });
