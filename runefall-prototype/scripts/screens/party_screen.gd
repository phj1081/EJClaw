...
static func show(main) -> void:
	...
	main.panel(root, Vector2(70, 130), Vector2(850, 520), Color("#172033"))
	main.label(root, "편성 슬롯", Vector2(104, 156), Vector2(220, 34), 28)
	for i in range(4):
		var h := GameData.hero(main.party_indices[i])
		var card: Panel = main.panel(root, Vector2(108 + i * 196, 236), Vector2(166, 260), Color("#111827"))
		...
		main.button(card, "지정", Vector2(22, 216), Vector2(122, 34), Callable(main, "set_active_slot").bind(i), 16, Color("#364764"), "button_blue")
	...
	main.panel(root, Vector2(980, 130), Vector2(540, 520), Color("#172033"))
	main.label(root, "보유 캐릭터", Vector2(1014, 156), Vector2(220, 34), 28)
	for i in range(GameData.HEROES.size()):
		...
		main.label(b, h.name, Vector2(8, 86), Vector2(116, 24), 18, Color("#ffffff"), HORIZONTAL_ALIGNMENT_CENTER)
		main.label(b, h.tag, Vector2(8, 108), Vector2(116, 20), 14, Color("#b7c6e4"), HORIZONTAL_ALIGNMENT_CENTER)
...
static func show_launch_confirm(main) -> void:
	...
	for i in range(4):
		var h := GameData.hero(main.party_indices[i])
		var card: Panel = main.panel(root, Vector2(96 + i * 350, 190), Vector2(300, 400), Color("#172033"))
