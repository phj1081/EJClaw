...
Codex가 기본 봇으로 동작하고, Codex는 별도 서비스가 아니라 필요 시 내부 러너/역할 라우팅으로 사용됩니다.
...
```bash
DISCORD_BOT_TOKEN=...                # Codex 봇 토큰
CLAUDE_CODE_OAUTH_TOKEN=...          # 또는 ANTHROPIC_API_KEY=...
ASSISTANT_NAME=Codex                # 트리거 이름 (@Codex)
```
...
- Codex 서비스 문제: `logs/ejclaw.error.log` 또는 `journalctl --user -u ejclaw -f`
